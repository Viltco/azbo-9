...
class MaterialPurchaseRequisition(models.Model):
    ...
    so_id = fields.Many2one('sale.order', string='SO Ref')

    agree_count = fields.Integer(compute='get_agree_count')

    def get_agree_count(self):
        for rec in self:
            count = self.env['purchase.requisition'].search_count([('req_id', '=', self.id)])
            rec.agree_count = count

    def action_agree_view(self):
        return {
